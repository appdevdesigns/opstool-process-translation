...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/ProcessTranslation
        ...
        async.series([
            ...
            function(next){
                ...
                AD.log('<green>building</green> opstools/ProcessTranslation');

                AD.spawn.command({
                    command:'./cjs',
                    // options:[path.join('opstools', 'ProcessTranslation', 'build.js')],
                    options:[path.join('steal', 'buildjs'), 'OpsPortal', path.join('opstools', 'ProcessTranslation')],
shouldEcho:true,
                    // exitTrigger:'opstools/ProcessTranslation/production.css'
                })
                .fail(function(err){
                    AD.log.error('<red>could not complete opstools/ProcessTranslation build!</red>');
                    next(err);
                })
                .then(function(){
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'ProcessTranslation', 'production.js'), tag:'packages/OpsPortal-ProcessTranslation.js', replace:'OpsPortal/production.js'}
                ];
